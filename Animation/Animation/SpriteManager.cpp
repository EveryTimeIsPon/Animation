...
SpriteManager::SpriteManager(SDL_Renderer* renderer)
{
	...
	{
		// note(tommi): windows specific break function
		//   we should really not use it, but hey...
		__debugbreak();
	}
}
...
AnimatedSprite* SpriteManager::CreateAnimatedSprite(const std::string& filename)
{
	std::ifstream stream;
	...
	AnimatedSprite* animSprite = new AnimatedSprite(itr->second);
